...
// Theo-and-Co S38 v3: the 1s / 2s sub-tick approach (v1 / v2) fought the
// EQ client's built-in regen prediction and caused mana-bar back/forward
// jitter. Per-tick meditate + Chromie-dial scaling now lives inside the
// standard CalcXRegen branches in client_mods.cpp; the 6s tic_timer
// drives the whole regen path and the client handles visual smoothing
// as it expects to. Client::DoSmoothFastRegen lifted out entirely.
