...
DynamicZone* Client::CreateXPDZ(uint32 zone_id, uint32 version, const std::string& name, uint32 min_players, uint32 max_players)
{
	uint32_t duration = RuleI(Monomyth, XPDZLifetimeSeconds);
	DynamicZone dz{ zone_id, version, duration, DynamicZoneType::XPDZ };
	dz.SetName(name);
	dz.SetMinPlayers(min_players);
	dz.SetMaxPlayers(max_players);
	return DynamicZone::TryCreate(*this, dz, false);
