...
void Client::SendMembership() {
	if (m_ClientVersion >= EQ::versions::ClientVersion::TOB) {
		...
		mc->entrysize = 30;				// [DH_MEMBERSHIP_FIX] Number of membership setting entries below
		...
		mc->entries[21] = 0;            // [DH_MEMBERSHIP_FIX] MountKeyRingSlots
		mc->entries[22] = 0;            // [DH_MEMBERSHIP_FIX] IllusionKeyRingSlots
		mc->entries[23] = 0;            // [DH_MEMBERSHIP_FIX] FamiliarKeyRingSlots
		mc->entries[24] = 0;            // [DH_MEMBERSHIP_FIX] FamiliarAutoLeave
		mc->entries[25] = 0;            // [DH_MEMBERSHIP_FIX] HeroForgeKeyRingSlots
		mc->entries[26] = 0;            // [DH_MEMBERSHIP_FIX] unknown
		mc->entries[27] = 0;            // [DH_MEMBERSHIP_FIX] unknown
		mc->entries[28] = 0;            // [DH_MEMBERSHIP_FIX] unknown
		mc->entries[29] = 200;          // [DH_MEMBERSHIP_FIX] DragonHoardSlots
		mc->exit_url_length = 0;
		//mc->exit_url = 0; // Used on Live: "http://www.everquest.com/free-to-play/exit-silver"

		QueuePacket(outapp);

		// [DH_MEMBERSHIP_FIX] Send membership details (feature flags including Dragon's Hoard)
		auto detailsapp = new EQApplicationPacket(OP_SendMembershipDetails, sizeof(Membership_Details_Struct));
		QueuePacket(detailsapp);
		safe_delete(detailsapp);
