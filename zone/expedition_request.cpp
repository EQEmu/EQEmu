...
bool ExpeditionRequest::CheckMembersForConflicts(const std::vector<std::string>& member_names)
{
	...
	auto entries = DynamicZonesRepository::GetCharactersWithDz(database, member_names, static_cast<int>(
		m_dz->IsRaidDZ() ? DynamicZoneType::RaidDZ :
		m_dz->IsXPDZ() ? DynamicZoneType::XPDZ :
