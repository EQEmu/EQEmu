...
public:
	...
	void RefreshOwnerRegenMult(); // Theo Phase 3 / S38: refresh cached owner OOC-regen state (dial bucket "regen_mult" + class + meditate skill). Same 6s tic + spawn cadence as the Drill-Master mults; NEVER call from a per-tick / per-hit path. The cached values are read in CalcRestState to compute RestRegenHP/Mana/Endurance.
	...
	// Theo-and-Co Phase 3 Group C (S1) — role is class-derived only (the
	// player-facing #bot role override was removed). GetEffectiveBotRole()
	// (defined in bot.cpp where bot_stat_model.h is in scope) returns the
	// class-default BotRole value; it is the group-queryable role API the
	// Group A stat model + future Group C reactive AI consume.
...
private:
	...
	// method calls for class/meditate). Bot::CalcRestState reads ONLY these
	// cached values -- never the DB -- to compute the meditate+dial-driven
	// RestRegenHP/Mana/Endurance. Same perf pattern as the Group A
	// Drill-Master mults. Defaults are safe (1.0x dial, class=0 falls into
	// the melee/Bard branch = flat 15s).
