...
bool Bot::Spawn(Client* botCharacterOwner) {
	if (
		...
		RefreshOwnerRegenMult();  // Theo Phase 3 / S38: prime cached owner regen state at spawn so the first CalcRestState reads real values, not defaults
...
void Bot::CalcRestState() {
	...
	// Theo-and-Co Phase 3 / S38 v3: OOC fast-regen via the OWNER's
	// meditate + Chromie dial (cached via RefreshOwnerRegenMult; this is
	// the only DB read on the bot regen path and it runs at 6s cadence,
	// not per-tick). The per-6s target equals the player-side formula:
	// maxPool * 6 / sec_to_full * dial. No multiplier pre-divide here
	// (unlike client_mods.cpp): the bot path uses
	// std::max(CalcXRegen(), RestRegenX), where CalcXRegen already
	// applies × HPRegenMultiplier/100 internally; RestRegenX is the
	// already-applied target so the max() compares apples to apples.
	float sec_to_full = TheoRegen::ComputeOOCSecondsToFull(
		m_owner_regen_class,
		m_owner_regen_meditate
	);
	if (sec_to_full > 0.0f) {
		const float per_6s_frac = 6.0f * m_owner_regen_mult / sec_to_full;
		RestRegenHP        = static_cast<unsigned int>(
			static_cast<float>(GetMaxHP()) * per_6s_frac);
		RestRegenMana      = static_cast<unsigned int>(
			static_cast<float>(GetMaxMana()) * per_6s_frac);
		RestRegenEndurance = static_cast<unsigned int>(
			static_cast<float>(GetMaxEndurance()) * per_6s_frac);
	}
}
...
//
// Called from the 6s Bot::Process tic + at spawn (NOT per hit). Touches
// the DB exactly once per call for the dial; class + meditate are cheap
// method calls on the owner.
//
...
// Theo-and-Co S38 v3: Bot::DoSmoothFastRegen REMOVED.
// The sub-tick fought the EQ client's regen prediction and caused
// mana-bar back/forward jitter. Per-tick meditate + Chromie-dial scaling
// lives in Bot::CalcRestState above (sets RestRegenHP/Mana/Endurance);
// Bot::Process's std::max(CalcXRegen, RestRegenX) picks it up on the
// standard 6s tic_timer cadence.
