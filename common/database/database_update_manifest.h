...
ALTER TABLE `character_pet_buffs`
	...
	ManifestEntry{
		.version = 9331,
		.description = "2026_07_28_achievement_content.sql",
		.check = "SELECT 1",
		.condition = "not_empty",
		.match = "",
		.sql = R"SQL(
...
FROM `task_reward_option_entries` LIMIT 0;
	...
	ManifestEntry{
		.version = 9332,
		.description = "2026_07_28_achievement_state.sql",
		.check = R"SQL(
SELECT 1
WHERE
	(SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			...
			AND column_name IN ('character_id', 'achievement_id', 'definition_version', 'completed_at')) < 4
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievements'
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'character_id,achievement_id'
	)
	OR (SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_progress'
			AND column_name IN ('character_id', 'achievement_id', 'component_type', 'component_sequence', 'component_id', 'current_count', 'completed', 'definition_version', 'updated_at')) < 9
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_progress'
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'character_id,achievement_id,component_type,component_id'
	)
	OR (SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_rewards'
			AND column_name IN ('character_id', 'achievement_id', 'reward_id', 'status', 'attempt_count', 'granted_at', 'last_attempt_at', 'last_error')) < 8
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_rewards'
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'character_id,achievement_id,reward_id'
	)
	OR (SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_reward_selections'
			AND column_name IN ('character_id', 'achievement_id', 'reward_set_id', 'selected_option_id', 'status', 'attempt_count', 'claimed_at', 'last_attempt_at', 'last_error')) < 9
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_reward_selections'
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'character_id,achievement_id,reward_set_id'
	)
	OR (SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			AND table_name = 'character_task_reward_instances'
			AND column_name IN ('occurrence_id', 'character_id', 'task_id', 'accepted_time')) < 4
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
			AND table_name = 'character_task_reward_instances'
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'occurrence_id'
	)
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'character_id,task_id'
	)
	OR NOT EXISTS (
	...
	OR (SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			...
			AND column_name IN ('pending_reward_id', 'character_id', 'task_id', 'accepted_time', 'source_instance_id', 'reward_set_id', 'selected_option_id', 'status', 'attempt_count', 'claimed_at', 'last_attempt_at', 'last_error')) < 12
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
			AND table_name = 'character_task_reward_selections'
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'pending_reward_id'
	)
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'character_id,source_instance_id'
	)
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.columns
		WHERE table_schema = DATABASE()
	...
	OR (SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			...
			AND column_name IN ('character_id', 'pending_reward_id', 'reward_id', 'status', 'attempt_count', 'granted_at', 'last_attempt_at', 'last_error')) < 8
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
			AND table_name = 'character_task_rewards'
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'pending_reward_id,reward_id'
	)
	OR (SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_pending_mutations'
			AND column_name IN ('mutation_id', 'character_id', 'source_target_type', 'source_target_id', 'operation', 'achievement_id', 'component_type', 'component_id', 'requested_value', 'definition_version', 'status', 'attempt_count', 'created_at', 'last_attempt_at', 'last_error')) < 15
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_pending_mutations'
		...
		HAVING MAX(non_unique) = 0
			...
			AND COUNT(column_name) = COUNT(*)
			AND SUM(CASE WHEN sub_part IS NULL THEN 0 ELSE 1 END) = 0
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'mutation_id'
	)
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.columns
		WHERE table_schema = DATABASE()
	...
	OR NOT EXISTS (
		...
		HAVING MAX(non_unique) = 1
			AND COUNT(*) = 3
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'character_id,status,mutation_id'
	)
	OR NOT EXISTS (
		SELECT 1 FROM information_schema.statistics
		WHERE table_schema = DATABASE()
		...
		HAVING MAX(non_unique) = 1
			AND COUNT(*) = 2
			AND GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',') = 'status,character_id'
	)
	OR NOT EXISTS (
	...
	OR (SELECT COUNT(*) FROM information_schema.columns
		WHERE table_schema = DATABASE()
			AND table_name = 'character_achievement_pending_mutations'
			AND (
				(column_name = 'mutation_id' AND data_type = 'bigint'
					AND column_type LIKE '%unsigned%' AND is_nullable = 'NO'
					AND column_default IS NULL
					AND LOWER(extra) LIKE '%auto_increment%')
				OR (column_name IN ('character_id', 'achievement_id', 'created_at')
					AND data_type = 'int' AND column_type LIKE '%unsigned%'
					AND is_nullable = 'NO' AND column_default IS NULL)
				OR (column_name IN ('component_id', 'requested_value',
						'definition_version', 'attempt_count', 'last_attempt_at')
					AND data_type = 'int' AND column_type LIKE '%unsigned%'
					AND is_nullable = 'NO' AND column_default = '0')
				OR (column_name IN ('source_target_type', 'operation')
					AND data_type = 'tinyint' AND column_type LIKE '%unsigned%'
					AND is_nullable = 'NO' AND column_default IS NULL)
				OR (column_name IN ('component_type', 'status')
					AND data_type = 'tinyint' AND column_type LIKE '%unsigned%'
					AND is_nullable = 'NO' AND column_default = '0')
				OR (column_name = 'source_target_id' AND data_type = 'bigint'
					AND column_type LIKE '%unsigned%' AND is_nullable = 'NO'
					AND column_default IS NULL)
				OR (column_name = 'last_error' AND data_type = 'varchar'
